...
(70.0) are close, confirming there is no overfitting to the validation folds.
Notably, average demand in the (more recent) test period is ~40% higher than in
training — the model captures this growth trend, which is why a naive
...
These are the choices that make the model correct rather than just functional:
...
The full narrated analysis lives in [`notebooks/01_analysis.ipynb`](notebooks/01_analysis.ipynb).
